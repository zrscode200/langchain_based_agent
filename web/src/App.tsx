...
import { AddWorkspace } from './add-workspace.tsx';
...
export default function App() {
  ...
  const [modal, setModal] = useState<'search' | 'settings' | 'actions' | 'help' | 'rename' | 'add-workspace' | null>(null);
  ...
  return <div className={'workspace ' + (sidebar ? '' : 'sidebar-hidden') + (panel ? ' has-panel' : '') + (inspectorSize.dragging ? ' resizing-panel' : '') + (panel && artifact && inspectorSize.overlayArtifact ? ' compact-artifact' : '') + (panel === 'tasks' && expandedTask ? ' expanded-task' : '')} style={{ '--inspector-width': `${inspectorSize.width}px` } as CSSProperties}>
    <aside className="sidebar" aria-label="Project and conversations">
      ...
      <div className="project-switcher"><span className="project-icon"><Layers size={19} /></span><label><span>WORKSPACE</span><select aria-label="Project" value={agent.projectId} onChange={e => { if (e.target.value === '__add_workspace__') setModal('add-workspace'); else if (e.target.value !== agent.projectId) agent.setProjectId(e.target.value); }}>{agent.projects.map(p => <option value={p.id} key={p.id}>{p.name}</option>)}<option disabled>──────────</option><option value="__add_workspace__">＋ Add workspace…</option></select></label><ChevronDown size={14} /></div>
