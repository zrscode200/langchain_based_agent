  ...
  const body = <div className="chat-timeline" key={scope}>
    {turns.map((turn, index) => {
      ...
      return <section className="chat-turn" key={turn.id} aria-label={`Turn ${index + 1}`}>
        {turn.user && <div className="user-message"><div className="turn-label">{userLabel}{turn.user.additional_kwargs?.__skill && <span className="pill">Invoked skill: {turn.user.additional_kwargs.__skill.name}</span>}<CopyButton text={textContent(turn.user.content)} /></div>{turn.user.additional_kwargs?.__skill ? <><Prose>{turn.user.additional_kwargs.__skill.args || `Invoked ${turn.user.additional_kwargs.__skill.name}`}</Prose><Disclosure storageKey={turn.user.id && 'skill:' + turn.user.id} title="Skill instructions sent"><Prose>{textContent(turn.user.content)}</Prose></Disclosure></> : <Prose>{textContent(turn.user.content)}</Prose>}</div>}
